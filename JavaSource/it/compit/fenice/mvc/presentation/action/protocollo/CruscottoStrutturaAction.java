...
public class CruscottoStrutturaAction extends Action {

	static Logger logger = Logger.getLogger(CruscottoStrutturaAction.class
			.getName());
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		boolean ufficioCompleto = false;
		if (form == null) {
			logger.info("Creating new Cruscotto Form");
			form = new CruscottoForm();
			session.setAttribute(mapping.getAttribute(), form);
		}
		cForm.setAbilitato(org.getUfficio(utente.getUfficioInUso())
				.isCaricaReferente(utente.getCaricaInUso()));
		...
		if (request.getParameter("impostaUfficioAction") != null) {
			...
			request.setAttribute("ufficioAssegnatarioId", cForm.getUfficioCorrente().getId());
			CaricaVO carica = CaricaDelegate.getInstance().getCaricaByUtenteAndUfficio(cForm.getSottoposto().getUtenteId(), cForm.getUfficioCorrente().getId());
			request.setAttribute("caricaAssegnatarioId", carica.getCaricaId());
