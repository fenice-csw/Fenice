...
public final class ReportStatisticheAction extends Action {
    ...
    List<UfficioVO> listaUffici = new ArrayList<UfficioVO> ();
	...
	public void stampaReport(HttpServletRequest request,
        ...
        try {
        	...
        	File reportFile = new File(context.getRealPath("/")+ FileConstants.STAMPA_STATISTICHE_PROTOCOLLI_REPORT_TEMPLATE);
            JasperDesign jasperDesign = JRXmlLoader
					.load(context.getRealPath("/")
							+ FileConstants.STAMPA_STATISTICHE_PROTOCOLLI_REPORT_TEMPLATE);
            ...
            Collection<StatisticheView> c = statistiche(UfficioId, dataInizio, dataFine, utente);
    ...
    private Collection<StatisticheView> statistiche(int ufficioSelezionatoId, Date dataDa,
            Date dataA, Utente utente) throws Exception {
        Organizzazione org = Organizzazione.getInstance();
        Collection<StatisticheView > statistiche = new ArrayList<StatisticheView >();

        StatisticheView st;
        try {
            listaUffici = new ArrayList<UfficioVO>();
            Collection<UfficioVO> uffi = getUffici(ufficioSelezionatoId);
            for (Iterator<UfficioVO> i = uffi.iterator(); i.hasNext();) {
                UfficioVO uff =  i.next();
                ...
                for (Iterator<Utente> y = ufficioCorrente.getUtenti().iterator(); y
                        .hasNext();) {
                    Utente ute =y.next();
                ...
                for (Iterator<UtenteVO> z = list.iterator(); z.hasNext();) {
                    UtenteVO ute = z.next();
    ...
    private Collection<UfficioVO> getUffici(int ufficioSelezionatoId) {
        Organizzazione org = Organizzazione.getInstance();
        try {
            if (org.getUfficio(ufficioSelezionatoId) != null
                ...
                if (org.getUfficio(ufficioSelezionatoId) != null
                    ...
                    for (Iterator<Ufficio> i = org.getUfficio(ufficioSelezionatoId)
                            .getUfficiDipendenti().iterator(); i.hasNext();) {
                        Ufficio uffCorr = i.next();
