...
/*
 * Adjust the behavior of the dataProcessor to avoid styles and make it look
 * like FCKeditor HTML output.
 */
function configureHtmlOutput(ev) {
	var editor = ev.editor, dataProcessor = editor.dataProcessor, htmlFilter = dataProcessor
			&& dataProcessor.htmlFilter;
			...
			.addRules( {
				...
				if (element.name == 'img') {
					...
					if (style) {
						...
						var match = /(?:^|\s)width\s*:\s*(\d+)px/i.exec(style), width = match
								&& match[1];
 ...
 * 
 * @param Css
 *            style string (can include more than one color
 * @return Converted css style.
 */
function convertRGBToHex(cssStyle) {
	return cssStyle.replace(/(?:rgb\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\))/gi,
