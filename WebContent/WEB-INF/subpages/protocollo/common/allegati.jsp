...
<p>
  ...
  <html:submit styleClass="button" property="allegaDocumentoAction" value="Allega" title="Allega il file selezionato" />
  	...
  	<logic:equal name="protocolloForm" property="flagTipo" value="I">
   		<a id="various3" class="button"  href="<%= request.getContextPath()%>/scanagent2/websocket.jsp?formFileName=formFileUpload&actionProperties=allegaDocumentoScannAction&jSessID=<%=request.getSession().getId()%>&url=<%= request.getContextPath()%>/page/protocollo/ingresso/documento.do" >Scanner</a> 		
  	</logic:equal>
   	<logic:equal name="protocolloForm" property="flagTipo" value="U">
   		<a id="various3" class="button" href="<%= request.getContextPath()%>/scanagent2/websocket.jsp?formFileName=formFileUpload&actionProperties=allegaDocumentoScannAction&jSessID=<%=request.getSession().getId()%>&url=<%= request.getContextPath()%>/page/protocollo/uscita/documento.do" >Scanner</a>	
   	</logic:equal>
   	<logic:equal name="protocolloForm" property="flagTipo" value="P">
   		<a id="various3" class="button" href="<%= request.getContextPath()%>/scanagent2/websocket.jsp?formFileName=formFileUpload&actionProperties=allegaDocumentoScannAction&jSessID=<%=request.getSession().getId()%>&url=<%= request.getContextPath()%>/page/protocollo/posta_interna/documento.do" >Scanner</a>		
	</logic:equal>
</p>
